...
import { specialtyLabelMap } from "@/lib/constants";
				...
				<div className="flex items-center gap-2 px-4">
					...
					<Breadcrumb>
						<BreadcrumbList>
							...
							<BreadcrumbItem>
								<BreadcrumbPage>{doctor?.person?.name || "—"}</BreadcrumbPage>
							</BreadcrumbItem>
						</BreadcrumbList>
					</Breadcrumb>
				</div>
										...
										<div className="grid md:grid-cols-2 lg:grid-cols-3 border-b-0">
											...
											<div className="flex items-center gap-3 p-6 text-muted-foreground">
												...
												<div className="flex flex-col">
													...
													<span className="text-sm font-medium text-foreground">
														{doctor?.person?.document
															? maskCPF(doctor.person.document)
															: "—"}
													</span>
												</div>
											</div>
										...
										<div className="flex items-center gap-3 border-b p-6 text-muted-foreground lg:border-r lg:border-b-0">
											...
											<div className="flex flex-col">
												...
												<span className="text-sm font-medium text-foreground">
													...
													{doctor?.person?.address?.house || "—"}
													{doctor?.person?.address?.complement &&
														` — ${doctor?.person?.address?.complement}`}
												</span>
												<span className="text-xs text-muted-foreground">
													{doctor?.person?.address?.neighborhood || "—"}
												</span>
											</div>
										</div>
										...
										<div className="flex items-center gap-3 p-6 text-muted-foreground">
											...
											<div className="flex flex-col">
												...
												<span className="text-sm font-medium text-foreground">
													...
													{doctor?.person?.address?.state || "—"}
												</span>
												<span className="text-xs text-muted-foreground">
													CEP: {doctor?.person?.address?.zipCode || "—"}
												</span>
											</div>
										</div>
									</div>
