...
export const Route = createFileRoute("/_app/reprofiling/done/handling/")({
	component: HandlingReprofilingPage,
	beforeLoad: ({ context }) => {
		const { auth } = context;
		...
		if (!auth.hasPendingIntentFor("PATIENT")) {
			throw redirect({ to: "/" });
		}
	},
});

export default function HandlingReprofilingPage() {
